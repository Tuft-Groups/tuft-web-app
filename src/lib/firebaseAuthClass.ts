...
export default class FirebaseAuth {
  ...
  constructor(email?: string, password?: string) {
    this.email = email ?? getAuth().currentUser?.email ?? undefined;
  ...
  signInWithPhoneNumber = async (phoneNumber: string) => {
    const user = this.getCurrentUser();
    const recaptchaVerifier = new RecaptchaVerifier(getAuth(), "send-otp-button", { size: "invisible" });
    this.confirmationResult = await signInWithPhoneNumber(AUTH, phoneNumber, recaptchaVerifier);

    return this.confirmationResult;
  };

  confirmPhoneNumber = async (code: string) => {
    console.log({ confirmationResult: this.confirmationResult, code });
...
export const FIREBASE_AUTH_ERRORS: Record<string, string> = {
  ...
  "wrong-password": "The entered credentials are incorrect. Please try again.",
  ...
  "auth/user-not-found": "The entered credentials are incorrect. Please try again.",
